...
4. Read the created source note. `compile ingest` stamps `source_quality` and defaults thin or metadata-only notes to `status: seed`; verify that judgment. If the raw source is a stub, task list, navigation shell, metadata-only asset, or failed extraction shell, keep the note short rather than padding it; do not add a `> [!note]` callout to flag the gap.
