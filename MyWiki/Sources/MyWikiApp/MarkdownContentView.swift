...
private struct MarkdownWebView: NSViewRepresentable {
    ...
    private static var themeStyle: String {
        ...
        :root {
            ...
            --mywiki-surface-hover: \(cssColor(NSColor(EditorialPalette.surfaceHover)));
            --mywiki-border: \(borderColor);
            --mywiki-border-hover: \(cssColor(NSColor(EditorialPalette.borderHover)));
            --mywiki-link: \(link);
            --mywiki-accent: \(accent);
        }
        """
    }
    ...
    private static var configurationScript: String {
        ...
        do {
            ...
            if let json = String(data: data, encoding: .utf8) {
                return "window.MyWikiMarkdownConfig = \(json);"
            }
        } catch {
            assertionFailure("Failed to encode markdown configuration: \(error)")
        }
        return "window.MyWikiMarkdownConfig = {};"
    }
